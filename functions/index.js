...
/**
 * Migration Script: Migrate existing dates in the LIVE "registrations" collection to the Noon Anchor (12:00:00 UTC).
 * It will read `enrolledAt`, `studentIdAssignedAt`, and `courseCompletedAt`.
 * If they exist, it converts them to 12:00:00 UTC and saves them back.
 *
 * To run: Open the deployed function URL in a browser and append ?run=live
 */
exports.migrateDatesToNoonLive = onRequest({region: "europe-west3", maxInstances: 1}, async (req, res) => {
    // Biztonsági ellenőrzés az ÉLES adatbázishoz (szigorúbb)
    if (req.query.run !== 'live') {
        res.status(400).send("Az ÉLES script futtatásához add hozzá a '?run=live' paramétert az URL végéhez.");
        return;
    }
    ...
    try {
        ...
        res.status(200).send(`Migráció sikeres az ÉLES adatbázisban (registrations)! Összesen ${updatedCount} tanuló rekordja lett frissítve a ${totalDocs}-ból. A dátumok fixen 12:00 UTC-re álltak be.`);

    } catch (error) {
        logger.error("Hiba az ÉLES migrációs script futtatásakor:", error);
        res.status(500).send(`Hiba történt az ÉLES migráció során: ${error.message}`);
    }
});
