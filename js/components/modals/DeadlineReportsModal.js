...
const DeadlineReportsModal = ({ students, onClose, onStudentClick }) => {
    ...
    return html`
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col transform transition-all" onClick=${e => e.stopPropagation()}>
                                ...
                                ` : filteredStudents.map(student => {
                                    ...
                                    return html`
                                        <tr key="${student.id}" className="hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-indigo-600 hover:text-indigo-900 cursor-pointer" onClick=${() => {
