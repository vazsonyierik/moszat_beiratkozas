...
const BookingModal = ({ course, onClose, onBook, isTestView, isWaitlist }) => {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md transform transition-all my-8" onClick=${e => e.stopPropagation()}>
                <header className="p-4 sm:p-6 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
                    <h3 className="text-xl font-bold text-gray-800">${isWaitlist ? 'Feliratkozás várólistára' : 'Jelentkezés'}</h3>
                ...
                <div className="p-4 sm:p-6 bg-indigo-50 border-b border-indigo-100">
                    ...
                    ${isWaitlist && html`
                        <div className="mt-3 p-3 bg-yellow-50 text-yellow-800 text-sm rounded-md border border-yellow-200">
                            <strong>Figyelem!</strong> Ez a foglalkozás jelenleg betelt. Ha feliratkozol a várólistára, e-mailben értesítünk, amint felszabadul egy hely.
                        </div>
                    `}
                </div>
                ...
                <main className="p-4 sm:p-6">
                    ...
                    <form onSubmit=${handleSubmit} className="space-y-4">
                        ...
                        <div className="pt-4 flex justify-end gap-3">
                            ...
                            >
                                ${isSubmitting ? html`<span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></span> Küldés...` : (isWaitlist ? 'Feliratkozás' : 'Jelentkezem')}
                            </button>
                        </div>
                    </form>
                </main>
...
const StudentAppointmentsApp = () => {
    ...
    const handleBookAppointment = async (bookingData) => {
        try {
            if (bookingData.isWaitlist) {
                ...
                await joinWaitlistFn(bookingData);
                showToast('Sikeresen feliratkoztál a várólistára!', 'success');
            } else {
    ...
    const openModal = (course, waitlist = false) => {
        ...
        <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                            ...
                            <li key=${course.id} className=${`hover:bg-gray-50 transition-colors ${isFull ? 'opacity-75' : ''}`}>
                                <div className="px-4 py-4 sm:px-6 flex items-center justify-between flex-wrap gap-4">
                                    ...
                                    <div className="mt-4 sm:mt-0 ml-4 flex flex-col sm:flex-row gap-2">
                                        ...
                                        ` : html`
                                            <button
                                                onClick=${() => openModal(course, false)}
            ...
            ${selectedCourse && html`
                <${BookingModal} 
                    ...
                    onClose=${() => { setSelectedCourse(null); setIsWaitlistMode(false); }}
